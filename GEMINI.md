...
```
ToolSearch: select:mcp__BusinessVault__read-note,mcp__BusinessVault__search-vault
```

Then read the industry file:
```
mcp__BusinessVault__read-note
...
4. Update dashboard data — pull problem_description, metrics, gtm,
...
> **The rule:** Never build, enrich, or update a niche workflow from memory or assumptions. Always vault → SOP → build.
