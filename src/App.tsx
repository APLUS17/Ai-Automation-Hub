...
export default function App() {
  ...
  return (
    <div className={`font-sans min-h-screen pb-16 transition-colors duration-300 ${theme === "dark" ? "dark bg-[#050505] text-zinc-100" : "light bg-neutral-50 text-neutral-900"}`}>
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-7xl h-[450px] bg-gradient-to-b from-indigo-500/10 via-transparent to-transparent pointer-events-none blur-3xl z-0" />

      <div className="relative z-10 max-w-5xl mx-auto px-4 pt-6 pb-4">
        <header className="bg-white dark:bg-[#121214] border border-neutral-200 dark:border-zinc-800 rounded-2xl px-5 py-4 flex flex-col md:flex-row justify-between items-center gap-4 shadow-sm">
          <a
            href="#pick"
            onClick={(e) => { e.preventDefault(); handleBackToDirectory(); }}
            className="flex items-center gap-2.5 font-sans font-black text-lg tracking-tight hover:opacity-90 transition-opacity focus-visible:ring-2 focus-visible:ring-indigo-500 rounded-xl focus-visible:outline-none p-1"
          >
            <div className="w-9 h-9 bg-indigo-500/10 border border-indigo-500/30 rounded-xl flex items-center justify-center">
              <Sparkles className="w-5 h-5 text-indigo-600 dark:text-indigo-400" />
            </div>
            <div className="text-left leading-none">
              <span className="text-neutral-900 dark:text-white font-extrabold text-sm tracking-widest uppercase block">
                AI Automation Hub
              </span>
              <span className="text-[10px] text-neutral-500 dark:text-zinc-400 font-bold block mt-0.5 uppercase tracking-wider">
                WAT Framework Operations
              </span>
            </div>
          </a>

          <div className="flex items-center gap-3 md:gap-4 overflow-x-auto py-1 max-w-full">
            <div className={`flex items-center gap-2 text-xs font-bold transition-colors ${currentRoute === "#pick" ? "text-indigo-600 dark:text-indigo-400" : "text-neutral-500 dark:text-zinc-400"}`}>
              <div className={`w-6 h-6 rounded-full flex items-center justify-center border text-[10px] font-black transition-all ${
                currentRoute === "#pick"
                  ? "bg-indigo-600/10 dark:bg-indigo-500/10 border-indigo-600 dark:border-indigo-500 text-indigo-600 dark:text-indigo-400"
                  : currentRoute === "#niche"
            ...
            <div className={`flex items-center gap-2 text-xs font-bold transition-colors ${currentRoute === "#niche" && activeTab === "outreach" ? "text-indigo-600 dark:text-indigo-400" : "text-neutral-500 dark:text-zinc-400"}`}>
              ...
              <span>Get Outreach</span>
            </div>
          </div>

          <div className="flex items-center gap-2">
            {/* n8n Connected Badge */}
            <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-emerald-500/10 border border-emerald-500/30 rounded-lg">
              <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
              <Workflow className="w-3 h-3 text-emerald-600 dark:text-emerald-400" />
              <span className="text-[10px] font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400">n8n Live</span>
            </div>
            <button
              onClick={toggleTheme}
              className="w-10 h-10 rounded-xl border border-neutral-200 hover:border-neutral-300 dark:border-zinc-700 dark:hover:border-zinc-600 flex items-center justify-center text-neutral-600 hover:text-neutral-900 dark:text-zinc-300 dark:hover:text-white transition-colors cursor-pointer bg-white dark:bg-zinc-900/40 shadow-sm focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:outline-none"
          ...
          {currentRoute === "#pick" ? (
            ...
            >
              <div className="text-center md:text-left mb-2">
                <h1 className="text-3xl md:text-4xl font-extrabold text-neutral-900 dark:text-white tracking-tight leading-none uppercase">
                  Select <span className="text-indigo-600 dark:text-indigo-400 font-black tracking-widest text-lg md:text-2xl bg-indigo-600/10 dark:bg-indigo-500/10 px-3 py-1.5 rounded-xl border border-indigo-600/20 dark:border-indigo-500/20 ml-1">Target Industry</span>
                </h1>
                <p className="text-neutral-600 dark:text-zinc-300 font-medium text-xs md:text-sm mt-3.5">
                  14 industries. 24 n8n workflow builds. Live market research and warm partner discovery powered by Gemini.
                </p>
              </div>

              <div className="relative">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-neutral-400 dark:text-zinc-400" />
                <input
                  type="text"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  placeholder="Search industries by name or keywords (e.g., 'hvac', 'coaching', 'healthcare', 'chiro')..."
                  className="w-full pl-12 pr-5 py-4 bg-white dark:bg-[#121214] border border-neutral-200 dark:border-zinc-800 hover:border-neutral-300 dark:hover:border-zinc-700 focus:border-indigo-600 dark:focus:border-indigo-500 rounded-2xl text-neutral-900 dark:text-zinc-100 placeholder-neutral-400 dark:placeholder-zinc-500 outline-none text-sm md:text-base transition-all focus:ring-4 focus:ring-indigo-600/10 dark:focus:ring-indigo-500/10 focus-visible:ring-2 focus-visible:ring-indigo-500 shadow-sm"
                />
              </div>
                ...
                {SEGMENTS.map(seg => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold border transition-all cursor-pointer focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:outline-none ${
                        isActive
                          ? "bg-indigo-600/10 dark:bg-indigo-500/10 border-indigo-600 dark:border-indigo-500 text-indigo-600 dark:text-indigo-400 shadow-sm font-bold"
                          : "bg-white dark:bg-[#121214] border-neutral-200 dark:border-zinc-800 text-neutral-600 dark:text-zinc-250 hover:text-neutral-900 dark:hover:text-white hover:bg-neutral-50 dark:hover:bg-zinc-800"
                      }`}
                    >
                      <span>{seg}</span>
                      {isActive && (
                        <span className="text-[10px] px-1.5 py-0.2 bg-indigo-600/20 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 font-extrabold rounded-full">
                          {count}
                        </span>
                      )}
                    </button>
